...
void print_cpoint_t(int ii,int jj,std::map<std::pair<int,int>,std::vector<cpoint_t>> map)
{
    ...
    std::vector<cpoint_t> clust_vec = map[pr];
    cout<<"Clusters: "<<endl;
    for(auto &vec : clust_vec)
        {
            cout<<"("<< vec.x <<", "<< vec.y <<", "<< vec.group <<"), ";
        }
    cout<<"No of Clusters: "<< map[pr].size()<<endl;
}

void print_hit_point_t(int ii,int jj,std::map<std::pair<int,int>,std::vector<hit_point_t>> map)
{
    ...
    std::vector<hit_point_t> clust_vec = map[pr];
    cout<<"hits: " <<endl;
    for(auto &vec : clust_vec)
        {
            cout<<"("<< vec.x <<", "<< vec.y <<", "<< vec.group <<"), ";
        }
    cout<<"No of Points: "<< map[pr].size()<<endl;
}

int cluster_number(int ii,int jj,std::map<std::pair<int,int> ,std::vector<cpoint_t>> map)
{
    std::pair<int,int> pr={ii,jj};
    return map[pr].size();
}

std::vector<int> cluster_size(int ii, int jj, std::map<std::pair<int,int> ,std::vector<cpoint_t>> map_cpoint, std::map<std::pair<int,int>,std::vector<hit_point_t>> map_hit_point)
{
    std::pair<int,int> pr = {ii,jj};
    std::vector<int> cluster_sizes;
    for(auto &cpt : map_cpoint[pr])
        {
            int count=0;
             for(auto &hpt : map_hit_point[pr])
                 {
...
int max_cluster_size(int events, int layers, std::map<std::pair<int,int> ,std::vector<cpoint_t>> map_cpoint, std::map<std::pair<int,int>,std::vector<hit_point_t>> map_hit_point)
{
    std::vector<int> cluster_sizes;
...
void Clustering() 
{
    ...
    TFile *output = new TFile("clustering.root","RECREATE");
    ...
    int max_cl_size = max_cluster_size(100,50,cluster_map_linkn,cluster_map);
    int max_layers = 50;
    ...
    for(int i=1; i<=max_layers; i++)
        {
            ...
            TH1F *h1 = new TH1F(hist_name.c_str(),hist_title.c_str(),max_cl_size+2,0,max_cl_size+2);

            for(int j=0; j<entries; j++)
                {
                    std::pair<int,int> pr = {j,i};
                    for(auto &vec : cluster_size(j,i,cluster_map_linkn,cluster_map))
                        {
                            h1->Fill(vec);
                        }
                    for(auto &vec : cluster_map_linkn[pr])
                        {
                            hx->Fill(vec.x);
                            hy->Fill(vec.y);
    ...
    for(int i=0; i<entries; i++)
        {
            ...
            for (int j=1; j<=max_layers; j++)
                {
                    std::pair<int,int> pr = {i,j};
                    count += cluster_map_linkn[pr].size();
                }
    ...
    for(int i=0; i<entries; i++)
        {
            for(int j=1; j<=max_layers; j++)
                {
                    if(j==1 && cluster_number(i,j,cluster_map_linkn)>2)
                    {break;}
                    else if(j>1 && cluster_number(i,j,cluster_map_linkn)!=1)
                    {break;}
